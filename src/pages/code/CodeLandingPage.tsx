...
import { ArrowRight, Sparkles, Zap, Globe, Code2, Database, Wand2, Smartphone, Monitor } from "lucide-react";
import { Helmet } from "react-helmet-async";
import { useTheme } from "next-themes";
import { StarsBackground } from "@/components/animate-ui/components/backgrounds/stars";
import { cn } from "@/lib/utils";

// Suggested prompts cleared — new ones will be added per request.
const TYPING_PROMPTS: string[] = [];
...
export default function CodeLandingPage() {
  ...
  const start = () => {
    ...
    if (seed) params.set("prompt", seed);
    params.set("platform", platform);
    navigate(`/code/new?${params.toString()}`);
  };

  return (
    <div className="relative min-h-screen overflow-x-hidden text-white selection:bg-white/20">
      <StarsBackground
      ...
      />
      <Helmet>
        <title>Code — Build your app from one prompt</title>
        <meta name="description" content="Code turns your ideas into real React apps with live preview and one-click publish, powered by AI." />
      </Helmet>
      ...
      <div className="font-body">
        ...
        <nav className="sticky top-0 z-50 bg-black/40 backdrop-blur-md border-b border-white/5">
          <div className="max-w-6xl mx-auto px-4 md:px-6 py-3 flex items-center justify-between">
            <Link to="/code" className="flex items-center gap-2">
              ...
              <span className="font-display font-bold tracking-tight text-lg">Code</span>
            </Link>
        ...
        <section className="relative px-4 md:px-6 pt-16 pb-12 max-w-5xl mx-auto">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            ...
            <p className="text-white/60 text-base md:text-lg max-w-xl leading-relaxed mb-8">
              Describe your idea in any language and Code will build a full React app with live preview and instant publish.
            </p>
          </motion.div>
              ...
              {PLATFORMS.map((p) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "inline-flex items-center gap-1.5 text-[11px] px-3 py-1.5 rounded-md border transition-colors",
                      active
          ...
          <div className="max-w-3xl mx-auto">
            ...
            <div className="space-y-10 md:space-y-12">
              {[
                ...
                { n: "02", t: "Watch the magic", d: "Code writes the app and shows it in an instant live preview." },
        ...
        <footer className="px-6 py-12 border-t border-white/5 text-center">
          ...
          <div className="text-[10px] text-white/20">© 2026 Code. Made with love.</div>
        </footer>
      </div>
    </div>
  );
