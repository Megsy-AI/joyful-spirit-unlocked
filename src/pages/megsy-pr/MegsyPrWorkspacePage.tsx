...
export default function MegsyPrWorkspacePage() {
  ...
  const deploy = async (mode: "preview" | "publish") => {
    ...
    try {
      ...
      if (mode === "publish") {
        ...
        // Save a stable cached screenshot URL so the home gallery shows a
        // real, static thumbnail of the published site instead of a blank
        // or a live preview each visit.
        const shotUrl = `https://s.wordpress.com/mshots/v1/${encodeURIComponent(data.url)}?w=1280&h=800`;
        supabase.from("projects").update({ thumbnail_url: shotUrl }).eq("id", projectId).then(() => {});
      }
    } catch (e) {
  ...
  // Mobile: avoid flashing the desktop layout on the base /build/:id route
  // while the redirect-to-/chat-or-/preview effect runs.
  if (isMobile && isBaseProjectRoute) {
    return (
