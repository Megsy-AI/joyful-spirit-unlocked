...
const VideoStudioPage = () => {
  ...
  const handleSend = async (promptOverride?: string) => {
    ...
    // Note: credit enforcement is server-side. Paid plans get unlimited video; server returns
    // a 402 with a clear error if truly insufficient. Avoids false positives during load.
          ...
          <div className="rounded-2xl bg-accent/40 backdrop-blur-sm">
            ...
            {settingsOpen && selectedModel && (
              <motion.div
                ...
                className="mx-3 mt-3 rounded-2xl border border-border/40 bg-background/40 backdrop-blur-xl p-4 space-y-5"
              >
                {/* Aspect */}
                <div>
            ...
            <div className="flex items-center gap-2 px-4 pb-4 flex-wrap">
              ...
              >
                ...
                {selectedModel && (
                  <span className="text-[10px] px-1.5 py-0.5 rounded-md border border-white/15 bg-white/10 text-foreground/85 font-semibold backdrop-blur-xl shadow-[inset_0_1px_0_rgba(255,255,255,0.12)]">
                    {(selectedModel.slug === "megsy-video" || selectedModel.slug === "megsy-video-i2v") ? "PRO" : `${currentCost} MC`}
                  </span>
                )}
              </button>
              <button
