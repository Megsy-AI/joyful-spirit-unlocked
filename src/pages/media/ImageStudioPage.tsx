...
const ImageStudioPage = () => {
  ...
  const { userId, refreshCredits } = useCredits();
  ...
  const handleSend = async (promptOverride?: string) => {
    ...
    // Note: credit enforcement happens server-side (paid plans = 0 credits, free trial for megsy-image,
    // otherwise spend_credits_auto). Client-side pre-check removed to avoid false "insufficient" errors
    // for paid users whose plan info is still loading or stored in a different account context.
      ...
      <div className="h-full flex flex-col bg-background relative overflow-hidden">
        ...
        <div className="relative z-10 p-3 bg-background/80 backdrop-blur-xl">
          <div className="rounded-2xl bg-accent/40 backdrop-blur-sm">
            {/* Settings panel — rendered FIRST so it expands UPWARD relative to the input bar */}
            {settingsOpen && selectedModel && (
              <div className="px-4 pt-4 pb-2 space-y-2 border-b border-border/30">
                <div>
            ...
            <div className="px-4 pt-4 pb-2">
              <textarea
                ...
                rows={6}
                className="min-h-[160px] max-h-[360px] w-full bg-transparent text-sm text-foreground outline-none resize-none placeholder:text-muted-foreground/40 leading-relaxed"
              />
            </div>
            ...
            <div className="flex items-center gap-2 px-4 pb-4 flex-wrap">
              ...
              >
                ...
                {selectedModel && <span className="text-[10px] px-1.5 py-0.5 rounded-md border border-white/15 bg-white/10 text-foreground/85 font-semibold backdrop-blur-xl shadow-[inset_0_1px_0_rgba(255,255,255,0.12)]">PRO</span>}
              </button>

              <button
                onClick={() => setSettingsOpen(o => !o)}
                className={`flex shrink-0 items-center gap-1.5 rounded-full px-3 py-2 text-xs font-medium transition-colors ${settingsOpen ? "bg-foreground text-background" : "bg-accent/60 text-muted-foreground hover:bg-accent hover:text-foreground"}`}
              >
