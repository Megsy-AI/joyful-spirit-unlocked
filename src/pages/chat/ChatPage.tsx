...
import iconMegsyOs from "@/assets/icon-megsy-os.png";
...
const getMegsyTierLabel = (tier: "lite" | "pro" | "max") => (
  tier === "lite" ? "Megsy Lite" : tier === "pro" ? "Megsy Pro" : "Megsy Max"
);

const CHAT_COMPOSER_MODEL_OPTIONS = [
  { kind: "tier" as const, id: "lite", label: "Megsy Lite", desc: "Fast everyday answers", premium: false },
  { kind: "tier" as const, id: "pro", label: "Megsy Pro", desc: "Smarter reasoning — unlimited on paid plans", premium: true },
  { kind: "tier" as const, id: "max", label: "Megsy Max", desc: "Flagship intelligence — unlimited on paid plans", premium: true },
  { kind: "model" as const, id: "openrouter:claude-sonnet-4-unlimited", label: "Claude Sonnet 4.6", desc: "Unlimited & free for paid subscribers", premium: true },
  { kind: "model" as const, id: "openrouter:claude-opus-4-unlimited", label: "Claude Opus 4.8", desc: "Unlimited & free for paid subscribers", premium: true },
];
...
const ChatPage = () => {
  ...
  const handleSendWithText = async (overrideText?: string) => {
    ...
    await streamChat({
      messages: allMessages, model: activeModel, tier: megsyTier as "lite" | "pro" | "max", searchEnabled: smartSearch,
                  ...
                  inlineSlot={
                    <>
                      <div className="relative">
                        ...
                        >
                          <span className="truncate max-w-[160px]">{selectedModel?.label || getMegsyTierLabel(megsyTier)}</span>
                          ...
                          {tierMenuOpen && (
                            <>
                              ...
                              <motion.div
                                ...
                                data-tier-menu
                                className="absolute bottom-full mb-2 right-0 z-[61] w-[min(280px,calc(100vw-24px))] max-w-[calc(100vw-24px)] rounded-2xl border border-border/60 bg-popover/95 backdrop-blur-xl shadow-[0_24px_48px_-12px_rgba(0,0,0,0.45)] p-1.5"
                              >
                                {CHAT_COMPOSER_MODEL_OPTIONS.map((item) => {
                                  const locked = item.premium && (userPlan === "free" || userPlan === "trial");
                                  const active = item.kind === "tier"
                                  ...
                                  return (
                                    <button
                                      ...
                                      onClick={() => {
                                        ...
                                        if (item.kind === "tier") {
                                          setSelectedModel(null);
                                          setMegsyTier(item.id as "lite" | "pro" | "max");
                                          if (chatUserId) {
                                            supabase.from("ai_personalization").upsert({ user_id: chatUserId, preferred_tier: item.id } as any, { onConflict: "user_id" }).then(() => {});
                                          }
                                        } else {
                                          setSelectedModel({ id: item.id, label: item.label, cost: 0 });
                                        }
                                        setTierMenuOpen(false);
                                      }}
                                    ...
                                    >
                                      <div className="flex-1 min-w-0">
                                        <div className="flex items-center gap-1.5 flex-wrap">
                                          <span className="text-[13px] font-semibold text-foreground/90">{item.label}</span>
                                          {item.premium && (
                                            <span className="text-[9px] font-semibold tracking-wide px-1.5 py-px rounded-md border border-white/15 bg-white/10 text-foreground/85 backdrop-blur-xl shadow-[inset_0_1px_0_rgba(255,255,255,0.12)]">PRO</span>
                                          )}
                                        </div>
                                      </div>
                      ...
                      {(chatMode === "slides" || chatMode === "slides-images" || selectedAgent?.id === "slides" || selectedAgent?.id === "slides-images") ? (
                        <button
                          type="button"
                          ...
                          <AnimatePresence>
                            {researchDepthOpen && (
                              <>
                                ...
                                >
                                  {([
                                    { id: "lite" as const, label: "Lite", desc: "Short report" },
                                    { id: "medium" as const, label: "Medium", desc: "Standard report" },
                                    { id: "max" as const, label: "Max", desc: "Comprehensive report" },
                                  ]).map(d => {
