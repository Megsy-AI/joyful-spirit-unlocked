...
export function FalModelPickerSheet(props: ImageProps | VideoProps) {
  ...
  const glassBadge = "inline-flex items-center rounded-md border border-white/15 bg-white/10 px-1.5 py-0.5 text-[10px] font-medium text-foreground/85 backdrop-blur-xl shadow-[inset_0_1px_0_rgba(255,255,255,0.12)]";
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto px-4 pb-8 space-y-2">
              {sorted.map((m: any) => {
                ...
                const isMegsyVideo = props.kind === "video" && (m.slug === "megsy-video" || m.slug === "megsy-video-i2v");
                const priceLabel =
                  props.kind === "image"
                    ? "PRO"
                    : isMegsyVideo
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1.5">
                          ...
                          {isMegsyVideo && <span className={glassBadge}>PRO</span>}
                        </div>
                        {m.description && (
                      ...
                      {/* Price */}
                      <span
                        className={`${props.kind === "image" || isMegsyVideo ? glassBadge : `text-[12px] font-black tabular-nums shrink-0 ${selected ? "text-primary" : "text-foreground/85"}`}`}
                      >
                        {priceLabel}
                      </span>
                    </div>
