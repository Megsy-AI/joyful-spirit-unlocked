...
const MODEL_LOGOS: Record<string, string> = {
  ...
  // Video models
  "megsy-video": "/model-logos/megsy.webp",
  "megsy-video-i2v": "/model-logos/megsy.webp",
              ...
              <div className="flex-1 overflow-y-auto p-3">
                <div className="grid grid-cols-2 gap-2">
                  {models.map((model) => {
                    ...
                    const isMegsyVideo = model.id === "megsy-video" || model.id === "megsy-video-i2v";
                    const subscriberOnly = mode === "images" || isMegsyVideo;

                    // iOS 26 glass badge style
                    const glassBadge = "text-[9px] px-1.5 py-0.5 rounded-md border border-white/15 bg-white/10 text-foreground/85 font-medium backdrop-blur-xl shadow-[inset_0_1px_0_rgba(255,255,255,0.12)]";
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2 mb-2 flex-wrap">
                          ...
                          {isNew && (
                            <span className={glassBadge}>NEW</span>
                          )}
                          {isMegsyVideo && (
                            <span className={glassBadge}>PRO</span>
                          )}
                        </div>
                        ...
                        <div className="flex items-center justify-between gap-2">
                          <div className="flex items-center gap-1 flex-wrap">
                            {badges.map((badge) => (
                              <span key={badge} className={glassBadge}>{badge}</span>
                            ))}
                          </div>
                          {subscriberOnly ? (
                            <span className={glassBadge}>PRO</span>
