...
import {
  Activity, UserPlus, UserMinus, Settings2, Wallet, Mail, ShieldCheck,
  Crown, Pause, Play, Trash2, Pencil, LogIn, type LucideIcon,
} from "lucide-react";
...
function iconFor(action: string): { Icon: LucideIcon; tint: string } {
  ...
  if (a.includes("invite")) return { Icon: Mail, tint: "text-amber-500 bg-amber-500/10 ring-amber-500/20" };
  if (a.includes("join") || a.includes("accept")) return { Icon: LogIn, tint: "text-emerald-500 bg-emerald-500/10 ring-emerald-500/20" };
  if (a.includes("add") && a.includes("member")) return { Icon: UserPlus, tint: "text-emerald-500 bg-emerald-500/10 ring-emerald-500/20" };
  if (a.includes("remove") || a.includes("delete")) return { Icon: UserMinus, tint: "text-destructive bg-destructive/10 ring-destructive/20" };
  if (a.includes("role") || a.includes("permission")) return { Icon: ShieldCheck, tint: "text-blue-500 bg-blue-500/10 ring-blue-500/20" };
  if (a.includes("owner")) return { Icon: Crown, tint: "text-amber-500 bg-amber-500/10 ring-amber-500/20" };
  if (a.includes("suspend")) return { Icon: Pause, tint: "text-orange-500 bg-orange-500/10 ring-orange-500/20" };
  if (a.includes("unsuspend") || a.includes("reactivate")) return { Icon: Play, tint: "text-emerald-500 bg-emerald-500/10 ring-emerald-500/20" };
  if (a.includes("credit") || a.includes("topup") || a.includes("payment") || a.includes("billing")) return { Icon: Wallet, tint: "text-emerald-500 bg-emerald-500/10 ring-emerald-500/20" };
  if (a.includes("trash")) return { Icon: Trash2, tint: "text-destructive bg-destructive/10 ring-destructive/20" };
  if (a.includes("update") || a.includes("edit") || a.includes("rename")) return { Icon: Pencil, tint: "text-foreground bg-foreground/[0.06] ring-border" };
  if (a.includes("setting") || a.includes("config")) return { Icon: Settings2, tint: "text-foreground bg-foreground/[0.06] ring-border" };
...
export default function ActivityTab() {
  ...
  const filtered = useMemo(() => {
    ...
    return logs.filter((l) => {
      ...
      if (filter === "billing" && !/(credit|topup|payment|billing|invoice|plan)/.test(a)) return false;
      if (filter === "settings" && !/(setting|config|rename|update|brand|notification|security|data)/.test(a)) return false;
                  ...
                  return (
                    <div key={l.id} className="px-4 py-3.5 hover:bg-foreground/[0.02] transition-colors">
                      <div className="flex items-start gap-3">
                        <div className={`w-8 h-8 rounded-lg grid place-items-center ring-1 shrink-0 ${tint}`}>
