...
// Transparent fallback — keeps the previous page visible until the next chunk
// is ready, so quick navigations feel instant instead of flashing a loader.
// PageLoader is kept as an export-only import for the initial app boot path.
void PageLoader;
const LazyFallback = () => null;

// Preload the most-used route chunks during browser idle time so that
// in-app navigations resolve from cache and never show the suspense fallback.
const preloadCommonRoutes = () => {
  const tasks: Array<() => Promise<unknown>> = [
    ...
    () => import("./pages/code/CodeLandingPage"),
    () => import("./pages/megsy-pr/MegsyPrHomePage"),
    () => import("./pages/community/CommunityPage"),
  ];
  const run = () => { tasks.forEach((t) => { t().catch(() => {}); }); };
  const ric = (window as unknown as { requestIdleCallback?: (cb: () => void) => void }).requestIdleCallback;
  if (typeof ric === "function") ric(run);
  else setTimeout(run, 1200);
};
if (typeof window !== "undefined") {
  if (document.readyState === "complete") preloadCommonRoutes();
  else window.addEventListener("load", preloadCommonRoutes, { once: true });
}
