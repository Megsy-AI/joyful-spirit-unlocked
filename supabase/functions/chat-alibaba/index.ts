...
serve(async (req) => {
  ...
  const rawSel = (typeof body?.selectedModel?.id === "string" ? body.selectedModel.id : "").toLowerCase();
  const isClaudeOpus = rawSel.includes("claude-opus");
  const isClaudeSonnet = rawSel.includes("claude-sonnet");
  const claudeFlavor = isClaudeOpus ? "Claude Opus 4.8" : isClaudeSonnet ? "Claude Sonnet 4.6" : null;
  const claudePrompt = claudeFlavor ? [
    `You are ${claudeFlavor}, a thoughtful, careful, and articulate AI assistant.`,
    "Reason step-by-step internally, then give a clear, well-structured answer.",
    "Prefer nuance, calibrated confidence, and concrete examples. Avoid hype and filler.",
    "Format complex answers with short paragraphs, lists, and headings when useful.",
  ...
  const systemPrompt = deepResearch ? researchPrompt : (claudePrompt || basePrompt);

  const selectedModelId = typeof body?.selectedModel?.id === "string" ? body.selectedModel.id : "";
  const chosenModel = deepResearch
    ? "qwen-max"
    : (claudeFlavor ? "qwen-plus-latest" : pickQwenModel(selectedModelId || body?.model || body?.tier, body?.tier, messages));
  ...
  if (authedUserId && supabaseUrl && serviceRole) {
    try {
      ...
      const [accs, toggles, mem] = await Promise.all([
        useToolsAllowed
          ? admin.from("pipedream_accounts").select("app_slug, account_id, external_user_id, healthy").eq("user_id", authedUserId)
          : Promise.resolve({ data: [] as any[], error: null }),
        useToolsAllowed
          ? admin.from("pipedream_tool_settings").select("app_slug, enabled").eq("user_id", authedUserId)
          : Promise.resolve({ data: [] as any[], error: null }),
        admin.from("user_memory_entries").select("title, summary").eq("user_id", authedUserId).order("created_at", { ascending: false }).limit(5),
      ]);
      ...
      recallSnippets = (mem.data ?? []).map((m: any) => `${m.title}: ${m.summary}`.slice(0, 300));
    } catch (e) {
      console.warn("[chat-alibaba] context fetch failed", e);
    }
  }

  const { integrationTools, integrationByName } = buildIntegrationCatalog(connectedAccounts, disabledSlugs);
  ...
  const activeSkill = body?.activeSkill && typeof body.activeSkill === "object" ? body.activeSkill : null;
  ...
  const finalSystemPrompt = claudePrompt
    ? `${claudePrompt}\n\n---\n\n${odysseusPrompt}`
    : odysseusPrompt;
