...
Deno.serve(async (req) => {
  ...
  // Plan-based unlimited:
  //  - Megsy Video → unlimited for Elite & Business (has_elite_plan).
  //  - Other models → normal credit deduction.
  let creditsCharged = credits;
  const isMegsyVideo = body.model_slug === "megsy-video";
  let eliteUnlimited = false;
  if (isMegsyVideo) {
    const { data: elite } = await admin.rpc("has_elite_plan", { p_user_id: user.id });
    eliteUnlimited = !!elite;
  }
  ...
  if (eliteUnlimited) {
    ...
    if (deduct && deduct.success === false) {
      return json({
        error: deduct.error || "insufficient_credits",
        credits: deduct.credits,
        message: isMegsyVideo
          ? "Megsy Video is unlimited on Elite and Business plans. Upgrade to unlock unlimited video."
          : undefined,
      }, 402);
    }
  }
  ...
  try {
    ...
    const a = await saveRemoteAsset({
      ...
      prompt: body.prompt, costCredits: creditsCharged,
    ...
    return json({ video_url: finalUrl, credits_charged: creditsCharged, provider: "leonardo" });
  } catch (err) {
