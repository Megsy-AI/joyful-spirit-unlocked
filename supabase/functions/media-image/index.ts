...
Deno.serve(async (req) => {
  ...
  // Plan / free-trial logic:
  //  - Any paid plan (Pro+) → unlimited image generation, no credits deducted.
  //  - Free users on the "megsy-image" model → 3 free generations (tracked in profiles.image_free_uses).
  //  - Otherwise → normal credit deduction.
  const { data: paidPlan } = await admin.rpc("has_paid_plan", { p_user_id: user.id });
  const isPaid = !!paidPlan;
  const isFreeTrialModel = body.model_slug === "megsy-image";
  let creditsCharged = credits;

  if (isPaid) {
    creditsCharged = 0;
  } else if (isFreeTrialModel) {
    const { data: trial, error: trialErr } = await admin.rpc("consume_free_image_use", {
      p_user_id: user.id, p_limit: 3,
    });
    if (trialErr) return json({ error: `trial_error:${trialErr.message}` }, 500);
    if (trial && trial.success === false) {
      return json({
        error: "free_trial_exhausted",
        message: "You used your 3 free Megsy Image generations. Upgrade to Pro for unlimited images.",
        used: trial.used, limit: trial.limit,
      }, 402);
    }
    creditsCharged = 0;
  } else {
  ...
  try {
    ...
    for (const u of urls) {
      const a = await saveRemoteAsset({
        ...
        prompt: body.prompt, costCredits: creditsCharged / Math.max(urls.length, 1),
    ...
    return json({ image_urls: finalUrls, image_url: finalUrls[0], credits_charged: creditsCharged, provider: "leonardo" });
  } catch (err) {
